...
- **容器端口**: 30010 → 3000（`server` profile）
...
# 2. 服务器拉取最新镜像并重启（秒级，不编译）
ssh -i AL-SGP_8.219.160.126_id_ed25519 -p 21822 root@8.219.160.126 \
  "cd /www/wwwroot/chat.rcrc.eu.org/ChatUI && docker compose --profile server pull && docker compose --profile server up -d"
